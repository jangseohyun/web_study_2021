...
// 다양한 방법들 중 한 가지(그나마 쉬운 것을 고르고 고른)이다.
...
public class MyUtil
{
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		int numPerBlock = 10;
		//-- 페이징 처리 시 게시물 리스트 하단의 숫자를 10개씩 보여줄 것이다.
		
		int currentPageSetup;
		//--현재 페이지 기준으로 보여주는 숫자의 설정값
		//  (이 페이지를 기준으로 보여주는 숫자가 달라져야 하기 때문)
		int page;
		//--페이지를 직접 지정하여 이동하도록 하기 위한 변수 
		int n;
		...
		// 페이지 요청을 처리하는 과정에서
		// URL 경로의 패턴에 대한 처리
		// ex) List.jsp → List.jsp + 『?』 → List.jsp?
		// ex) List.jsp?pageNum=4 → List.jsp?pageNum=4 + 『&』 → List.jsp?pageNum=4&[   ]
		...
		//   『5 / 10 = 0』 이며, 여기에 『*10』(10을 곱)해도 0이다.
		//   하지만, 현재 페이지가 11페이지라면 (currentPage=11)
		//   『11 / 10 = 1』 이며, 여기에 『*10』(10을 곱)하면 10이다.
		...
		//   『20 / 10 = 2』이며, 여기에 『*10』(10을 곱)해서 20이 되는데
		...
		//   이때 현재 페이지(currentPage)가 11 페이지 이상일 경우
		//   『Prev』를 붙이기 위한 내용.
		// - 『Prev』를 클릭할 경우
		//   n 변수 페이지로 이동하는데 12에서 『Prev』할 경우 2 페이지가 되고
		...
		// 반복문 돌릴 때 최대 페이지보다 적고, 현재 페이지 설정값에 10을 더한 값보다 적은 동안만 돌린다.
